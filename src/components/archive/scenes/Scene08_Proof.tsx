...
export default function Scene08_Proof() {
  ...
  const LINKS = [
    { label: "How It Works", href: "/archive/how-it-works" },
    { label: "State Vector", href: "/archive/state-vector" },
    { label: "Reward Function", href: "/archive/reward-function" },
    { label: "Validation", href: "/archive/validation" },
  ];
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-4xl">
        ...
        >
          Evaluated on the held-out {content.training.test_split} test seasons (data the model never saw)
        </motion.p>
          ...
          {criteria.map((c, i) => (
            ...
            >
              ...
              {c.id === "reward_improvement" && (
                <div>
                  <p className="text-3xl font-bold font-mono text-brand-gold">
                    <AnimatedNumber value={c.percentage_change || 0} decimals={1} prefix="+" suffix="%" />
                  </p>
                  <p className="text-xs text-white/30 mt-1 font-mono">
                    {c.early_value} to {c.late_value}
                  </p>
                </div>
              )}
              ...
              {c.id === "pull_rate_differential" && (
                <div>
                  <p className="text-3xl font-bold font-mono text-brand-gold">
                    <AnimatedNumber value={c.differential_percent || 0} decimals={0} prefix="+" suffix="%" />
                  </p>
                  <p className="text-xs text-white/30 mt-1 font-mono">
              ...
              {c.id === "case_studies" && (
                <div>
                  <p className="text-3xl font-bold font-mono text-brand-gold">
                    <AnimatedNumber value={c.count || 0} decimals={0} />
                  </p>
